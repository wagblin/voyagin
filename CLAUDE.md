...
- **Mobile** : EAS Update (pas de build natif ni de compte Apple Developer — gratuit). Projet EAS `@wagblin/voyagin` (`eas init`). Publication manuelle : `EXPO_PUBLIC_API_URL=<url backend> eas update --branch production --message "..."` depuis `packages/mobile`. Publication automatique en CI : `.github/workflows/eas-update.yml` (nécessite le secret GitHub `EXPO_TOKEN`, un token créé sur expo.dev). Ouvrable dans Expo Go via la page du projet sur expo.dev, sans lancer Metro localement — **contrairement à backend/web, il faut republier explicitement après un changement de code mobile, ce n'est pas automatique tant que la CI n'est pas configurée**.

Non fait / à décider avec l'utilisateur si besoin : domaine custom, passage de Railway en plan payant (le crédit d'essai gratuit expire).
